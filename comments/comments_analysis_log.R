...
SC2analysis <- renderUI({fixedRow(column(12,
      "This check is not required"))})

#----SC3 Variable names and units----
SC3analysis <- renderUI({fixedRow(column(12,
...
SC4analysis <- renderUI({fixedRow(column(12,
      "Analysis comments",
      br(), br(),
...
SC5analysis <- renderUI({fixedRow(column(12,
...
#----SC6 Accessibility----
SC6analysis <- renderUI({fixedRow(column(12,
      "This check is not required"))})

#----SC7 Caveats and footnotes----
SC7analysis <- renderUI({fixedRow(column(12,
      "This check is not required"))})

#----SC8 Output formatting----
SC8analysis <- renderUI({fixedRow(column(12,
      "This check is not required"))})

#----SC9 RAP----
SC9analysis <- renderUI({fixedRow(column(12,
      "This check is not required"))})
...
VE2analysis <- renderUI({fixedRow(column(12,
      "Usability testing",
      br(), br(),
      "Can a new user easily re-run the analysis and view outputs?"))})

#----VE3 Autochecks----
VE3analysis <- renderUI({fixedRow(column(12,
      "Autochecks",
      br(), br(),
...
VE4analysis <- renderUI({fixedRow(column(12,
      "This check is not required."))})

#----VE5 Use of model outputs----
VE5analysis <- renderUI({fixedRow(column(12,
      "This check is not required."))})

#----VE6 Visual correctness----
VE6analysis <- renderUI({fixedRow(column(12,
      "This check is not required."))})

#----Validation----

#----VA1 Methodology correctness----
VA1analysis <- renderUI({fixedRow(column(12,
...
VA2analysis <- renderUI({fixedRow(column(12,
...
VA3analysis <- renderUI({fixedRow(column(12,
      "Uncertainty testing",
      br(), br(),
...
VA4analysis <- renderUI({fixedRow(column(12,
      "This check is not required."))})

#----VA5 Re-performance testing----
VA5analysis <- renderUI({fixedRow(column(12,
      "This check is not required."))})
...
DA2analysis <- renderUI({fixedRow(column(12,
      "Data transformation",
      br(), br(),
...
#----DA3 Assumptions----
DA3analysis <- renderUI({fixedRow(column(12,
      "Assumptions",
      br(), br(),
      "Does your assumptions log contain details of all analytical assumptions?"))})

#----DA4 Decisions----
DA4analysis <- renderUI({fixedRow(column(12,
      "This check is not required."))})